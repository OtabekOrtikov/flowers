...
for (let g = 0; g < contBtn.length; g++) {
    contBtn[g].addEventListener('click', (event) => {
        let current = document.querySelector('.shop-filter__category-list .active');
        if (current) {
...
function changePage(page) {
    ...
    function checkActivity() {
        let check = document.querySelector('.shop-filter__category-list .active').id;
        if (check == 'all') {
            data = data;
        } else if (check == 'mom') {
            data = data.filter(item => item.category == 'mom');
        } else if (check == 'marry') {
            data = data.filter(item => item.category == 'marry');
        } else if (check == 'classic') {
            data = data.filter(item => item.category == 'classic');
        } else if (check == 'children') {
            data = data.filter(item => item.category == 'children');
        } else {
            data = 'data';
        }
        return data;
